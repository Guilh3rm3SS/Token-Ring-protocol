...
def get_pod_id():
    ...
    match = re.search(r'store-node-(\d+)', hostname)
    return int(match.group(1)) if match else -1

STORE_ID = get_pod_id()
MY_STORE_ADDR = f"tcp://store-node-{STORE_ID}.store-service.default.svc.cluster.local:{STORE_PORT}"
...
def propagar_para_backups_sync(conteudo):
    ...
    for i in range(3):
        ...
        addr = F"tcp://store-node-{i}.store-service.default.svc.cluster.local:{STORE_PORT}"
        try:
