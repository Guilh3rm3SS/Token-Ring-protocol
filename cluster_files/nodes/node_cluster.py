...
MY_TOKEN_ADDR = f"tcp://*:{TOKEN_PORT + NODE_ID}"
NEXT_NODE = f"tcp://cluster-node-{(NODE_ID + 1) % TOTAL_NODES}:{TOKEN_PORT + ((NODE_ID + 1) % TOTAL_NODES)}"
CLIENT_RECEIVE = f"tcp://*:{CLIENT_PORT + NODE_ID}"
CLIENT_SEND = f"tcp://client-node-{NODE_ID}:{CLIENT_PORT + NODE_ID}"
...
def process_and_ack(message):
    ...
    for attempt, store_id in enumerate(store_ids[:MAX_RETRIES]):
        store_addr = f"tcp://cluster-store-{store_id}:{8000}"
